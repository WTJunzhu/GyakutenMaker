...
# valid Ren'Py public APIs (renpy.say, renpy.call_screen, etc.).
#
# Architecture:
...
init -996 python in _aa_rt:
    ...
    def _say_line(line):
        ...
        Speak one dialogue line dict:
          ...
          { "text": "..." }  (narrator)
        Uses renpy.say() — a fully valid public API.
        """
        ...
        if char_id:
            ...
            if char is not None:
                ...
                renpy.say(None, "[{}] {}".format(char_id, text))
        else:
            renpy.say(None, text)
    ...
    def _apply_scene(node):
        ...
        if scene:
            # e.g. "bg courtroom" → renpy.scene() + show
            # We use renpy.scene() with the image tag then show the bg.
            ...
            if len(parts) == 2 and parts[0] == "bg":
                try:
                    renpy.show(parts[1], tag="bg", layer="master")
                except Exception:
                    pass
            elif len(parts) == 1:
                try:
                    renpy.show(parts[0], tag=parts[0], layer="master")
                except Exception:
                    pass
        ...
        if bgm:
            try:
                renpy.music.play(bgm, channel="music")
            except Exception:
                pass
    ...
    def dispatch(node, case_data):
        ...
        if ntype == "dialogue":
            ...
            renpy.say(None, "[AA Runtime] Unknown node type: {}".format(ntype))
    ...
    def exec_dialogue(node, case_data):
        ...
        if node.get("show_health_bar"):
            renpy.show_screen("aa_health_bar")

        _say_lines(node.get("lines", []))

        if node.get("hide_health_bar"):
            renpy.hide_screen("aa_health_bar")
    ...
    def exec_get_evidence(node, case_data):
        ...
        for ev_id in node.get("evidence_ids", []):
            ...
            if ev_data is not None:
                ...
                renpy.say(None, "[AA Runtime] Unknown evidence: {}".format(ev_id))
        return node.get("next")
            ...
            if action.startswith("present:"):
                ...
                if selected in correct_ids:
                    ...
                    if store._aa_health <= 0:
                        ...
                        renpy.hide_screen("aa_testimony_panel")
                        renpy.hide_screen("aa_press_present_bar")
                        renpy.hide_screen("aa_courtroom_keys")
                        return None
                    continue
    ...
    def exec_choice(node, case_data):
        ...
        choice_idx = renpy.call_screen("aa_choice_menu",
                                       prompt=prompt,
